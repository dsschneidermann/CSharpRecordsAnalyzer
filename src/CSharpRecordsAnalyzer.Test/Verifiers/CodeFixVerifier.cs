...
namespace CSharpRecordsAnalyzer.Test.Verifiers
{
    /// <summary>
    ...
    public abstract class CodeFixVerifier : DiagnosticVerifier
    {
        ...
        protected void VerifyBasicFix(
            string oldSource, string newSource, int? codeFixIndex = null, bool allowNewCompilerDiagnostics = false)
        {
            VerifyFix(
                LanguageNames.VisualBasic, GetBasicDiagnosticAnalyzer(), GetBasicCodeFixProvider(), oldSource,
        ...
        {
            VerifyFix(
                LanguageNames.CSharp, GetCSharpDiagnosticAnalyzer(), GetCSharpCodeFixProvider(), oldSource, newSource,
        ...
        private void VerifyFix(
            string language, DiagnosticAnalyzer analyzer, CodeFixProvider codeFixProvider, string oldSource,
            string newSource, int? codeFixIndex, bool allowNewCompilerDiagnostics)
        {
            ...
            var analyzerDiagnostics =
                Helpers.DiagnosticVerifier.GetSortedDiagnosticsFromDocuments(analyzer, new[] {document});
            ...
            for (var i = 0; i < attempts; ++i)
            {
                ...
                var context = new CodeFixContext(
                    document, analyzerDiagnostics[0], (a, d) => actions.Add(a), CancellationToken.None
                );
                ...
                var newCompilerDiagnostics = Helpers.CodeFixVerifier.GetNewDiagnostics(
                    compilerDiagnostics, Helpers.CodeFixVerifier.GetCompilerDiagnostics(document)
                );
                ...
                if (!allowNewCompilerDiagnostics && newCompilerDiagnostics.Any())
                {
                    ...
                    document = document.WithSyntaxRoot(
                        Formatter.Format(
                            document.GetSyntaxRootAsync().Result, Formatter.Annotation,
                            document.Project.Solution.Workspace
                        )
                    );
                    newCompilerDiagnostics = Helpers.CodeFixVerifier.GetNewDiagnostics(
                        compilerDiagnostics, Helpers.CodeFixVerifier.GetCompilerDiagnostics(document)
                    );
